...
{
	[System.Serializable]
	public class evento : UnityEngine.Events.UnityEvent<bool> { }
	public  bool isOn;
	...
	public evento eventi = new evento();
	...
	void Awake()
	{
		...
		float toggleSizeX = toggle.rect.x;
	...
	CanvasGroup Transparency (GameObject alphaObj, float startAlpha, float endAlpha)
	{
		...
		alphaVal.alpha = Mathf.Lerp(startAlpha, endAlpha, t += speed * Time.unscaledDeltaTime);
		return alphaVal;
	}
	...
	void StopSwitching()
	{
		if(t > 1.0f)
		{
			...
			if (isOn)
				...
				MoreMountains.NiceVibrations.MMVibrationManager.Errore();
		}
	}
